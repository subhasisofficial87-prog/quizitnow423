...
export default function ICSELecturePage() {
  ...
  if (loading) return <div className="min-h-screen flex flex-col items-center justify-center gap-4"><Loader2 className="w-12 h-12 animate-spin text-green-500" /><p className="font-nunito text-lg font-bold text-gray-900 dark:text-white">Generating your lecture...</p></div>;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => router.push(`/ICSE/study/${bookId}`)} className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900">
        ...
        <div className="flex items-center justify-between gap-4">
          {day > 1 && (
            <button onClick={() => router.push(`/ICSE/study/${bookId}/lecture/${day - 1}`)} className="flex items-center gap-2 px-5 py-3 border-2 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-semibold rounded-xl hover:bg-gray-50">
              <ArrowLeft className="w-4 h-4" />Day {day - 1}
            </button>
          )}
          <button onClick={() => router.push(`/ICSE/study/${bookId}/quiz/${day}`)} className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold rounded-xl shadow-md">
