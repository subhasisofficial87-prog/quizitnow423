...
export default function OdiaBoardDashboard() {
    ...
    const fetchData = async () => {
      try {
        const [meRes, booksRes, progressRes] = await Promise.all([
          fetch('/api/auth/me'),
          fetch('/api/books?board=odia_board'),
          fetch('/api/progress'),
        ]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Here&apos;s your Odia Board dashboard</p>
        </div>

        {/* Stats Row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Books', value: books.length, color: 'text-orange-600' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <a
                href="/OdiaBoard"
                className="flex items-center gap-1.5 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold rounded-xl transition-colors shadow-md"
              >
            ...
            {books.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-10 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Upload your first textbook to get started!</p>
                <a
                  href="/OdiaBoard"
                  className="inline-flex items-center gap-2 mt-4 px-5 py-2.5 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-colors"
                >
                  <Plus className="w-4 h-4" />
                  Upload Book
                </a>
              ...
              <div className="space-y-3">
                {books.map((book) => (
                  <div
                    key={book.id}
                    onClick={() => book.processing_status === 'completed' && router.push(`/OdiaBoard/study/${book.id}`)}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 text-xs text-gray-500 dark:text-gray-400">
                          <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded-full font-medium">
                            {classDisplayName(book.class_level)}
                          </span>
